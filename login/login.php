...
if (isset($_COOKIE['idclient']) && isset($_COOKIE['key'])) {
    $id = $_COOKIE['idclient'];
    $key = $_COOKIE['key'];

    // ambil username berdasarkan id
    $result = mysqli_query($conn, "SELECT link FROM client WHERE idclient = $id");
    $row = mysqli_fetch_assoc($result);

    //cek cookie dan username
    if ($key === hash('sha256', $row['link'])) {
        // code...
        $_SESSION['login'] = true;
    }
}

// mengembalikan ke halaman index.php ketika sudah login
...
if (isset($_POST["login"])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM client WHERE link = '$link'");
    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        if (password_verify($password, $row["password"])) {
            ...
            if(isset($_POST["remember"])){
                //buat cookie
                setcookie('idclient', $row['idclient'], time()+60);
                setcookie('key', hash('sha256', $row['link']), time()+60);
            }
    ...
    <header id="header" class="ex-2-header">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <h1>Log In</h1><br><br>
                   <!-- <p>Belum Punya Akun? <a class="white" href="home/sign-up.php">Daftar Sekarang</a></p> -->
                        ...
                        <form role="form" action="" method="post" name="login">
                            <fieldset>
                                ...
                                <div class="form-group">
                                    <input class="form-control" placeholder="link" name="link" type="text" id="link" autofocus required>
                                </div>
                                <div class="form-group">
